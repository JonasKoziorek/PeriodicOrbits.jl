...
function check_parameters(alg::DavidchackLai)
    if (alg.n < 1)
        throw(ArgumentError("`n` must be a positive integer."))
    end

    if !(1 <= alg.m <= alg.n)
        throw(ArgumentError("`m` must be in [1, `n`=$(alg.n)]"))
    end
end

function periodic_orbits(ds::DiscreteTimeDynamicalSystem, alg::DavidchackLai, igs::Vector{InitialGuess})
    if isinplace(ds)
